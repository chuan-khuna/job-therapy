...
const cardStyle: Record<CardState, React.CSSProperties> = {
  idle:    { border: "1px solid var(--color-border)",        background: "var(--color-bg)",      opacity: 0.38, boxShadow: "none" },
  partial: { border: "1px solid var(--color-border-strong)", background: "var(--color-bg)",      opacity: 0.65, boxShadow: "none" },
  matched: { border: "1px solid var(--color-accent)",        background: "var(--color-surface)", opacity: 1,
             boxShadow: "0 1px 2px oklch(20% 0.015 80 / 0.04), 0 4px 16px oklch(20% 0.015 80 / 0.06)" },
};

export default function TypeCard({ name, keys, matchedKeys = [], state = "idle", note }: TypeCardProps) {
  return (
    <div style={{ borderRadius: "12px", padding: "0.9rem 1rem", transition: "opacity 0.2s, border-color 0.2s, background 0.2s, box-shadow 0.2s", ...cardStyle[state] }}>
